...
from lio.env import room_symmetric_tax_only as room
...
def train_function(config, show_progress=False):
    ...
    if config.env.name == 'er':
        list_suffix = ['reward_total', 'extrinsic_reward', 'n_lever', 'n_door', 'externality']
    ...
    for idx_episode in range(1, n_episodes + 1):
        ...
        if idx_episode % period == 0:
            (reward_total, extrinsic_rewards, n_lever, n_door, steps_per_episode, _, _, externality, social_welfare) = evaluate.test_room_symmetric_tax_planner(n_eval, env, sess, list_agents)
            combined = np.stack([reward_total, extrinsic_rewards, n_lever, n_door, externality])
            s = '%d,%d,%d' % (idx_episode, step_train, step)
            for idx in range(env.n_agents):
                s += ',{:.3e},{:.3e},{:.3e},{:.3e},{:.3e}'.format(*combined[:, idx])
...
class TaxPlannerBuffer(object):
    ...
    def add(self, tax_planner_transition, infos=None):
        ...
        self.tax_planner_action[self.n_steps] = tax_planner_transition[1]
